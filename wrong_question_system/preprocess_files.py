...
from text_chunk import chunk_exam_file, extract_chunk_relationships
...
def process_test_paper(docx_file, config, class_name):
    try:
        ...
        # 文本分块与关系提取
        question_chunks = chunk_exam_file(text_with_captions)
...
def process_files(config, course):
    try:
        upload_file_path = config["upload_file_path"]
        geo_dir = os.path.join(upload_file_path, course)
        logger.info("开始处理 %s 文件夹", geo_dir)
        for filename in os.listdir(geo_dir):
            if filename.lower().endswith(".docx"):
                docx_file_path = os.path.join(geo_dir, filename)
                process_test_paper(docx_file_path, config, course)
    except Exception as e:
        logger.error("处理 geo 文件夹时出现错误: %s", e, exc_info=True)

def process_single_file(config, course, filename):
    try:
        # upload_file_path = config["upload_file_path"]
        # file_dir = os.path.join(upload_file_path, course)        
        # docx_file_path = os.path.join(file_dir, filename)
        logger.info("开始处理 %s 文件", filename)
        process_test_paper(filename, config, course)
    except Exception as e:
        logger.error("处理 geo 文件夹时出现错误: %s", e, exc_info=True)
